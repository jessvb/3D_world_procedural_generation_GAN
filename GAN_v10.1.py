# v10.1 fixes (?) the white border issue when generating images by increasing the size of the
# image and then cropping it down to the correct size again
# and fixes the issue with the discriminator loss blowing up by resetting the input images
...
# also note that it re-implements the biases and weights that were accidentally unused in prev iterations
...
# The width of the part of the image that doesn't include the white border (on a scale from 0 to 1):
WIDTH_TO_CROP = 244/IMAGE_SIZE
...
def generator(z, batch_size, z_dim, reuse=False):
    with tf.variable_scope('generator') as scope:
        ...
        # scale the image to crop out the white border
        scaled_image = tf.image.crop_and_resize(image=H_conv4,
                                                boxes=tf.constant([
                                                    1, [0, 0, WIDTH_TO_CROP, WIDTH_TO_CROP]], shape=[1, 4]),
                                                box_ind=tf.constant([1]),
                                                crop_size=tf.constant([
