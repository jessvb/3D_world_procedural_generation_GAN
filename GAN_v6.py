## v6 uses transpose upscaling and a 4x4 filter and different d/g alphas
...
BATCH_SIZE = 52  # todo
ITERATIONS = 7000 # todo
D_ALPHA = 3e-4  # the discriminator learning rate todo
G_ALPHA = 3e-3  # the generator learning rate todo
...
def generator(z, batch_size, z_dim, reuse=False):
    with tf.variable_scope('generator') as scope:
        ...
        STRIDES = [1,2,2,1]
        FILTER_SIZE =  4 # square filter of FILTER_SIZExFILTER_SIZE
        ...
        W_conv1 = tf.get_variable('g_wconv1', [FILTER_SIZE, FILTER_SIZE, output1_shape[-1], int(h0.get_shape()[-1])],
                                  initializer=tf.truncated_normal_initializer(stddev=0.1))
        b_conv1 = tf.get_variable(
            'g_bconv1', [output1_shape[-1]], initializer=tf.constant_initializer(.1))
        H_conv1 = tf.nn.conv2d_transpose(h0, W_conv1, output_shape=output1_shape,
                                         strides=STRIDES, padding='SAME') + b_conv1
        ...
        # Second DeConv Layer
        output2_shape = [batch_size, s4, s4, g_dim*2] # was [batch_size, s4 - 1, s4 - 1, g_dim*2]
        W_conv2 = tf.get_variable('g_wconv2', [FILTER_SIZE, FILTER_SIZE, output2_shape[-1], int(H_conv1.get_shape()[-1])],
                                  initializer=tf.truncated_normal_initializer(stddev=0.1))
        b_conv2 = tf.get_variable(
            'g_bconv2', [output2_shape[-1]], initializer=tf.constant_initializer(.1))
        H_conv2 = tf.nn.conv2d_transpose(H_conv1, W_conv2, output_shape=output2_shape,
                                         strides=STRIDES, padding='SAME') + b_conv2
        ...
        # Third DeConv Layer
        output3_shape = [batch_size, s2 - 1, s2 - 1, g_dim*1] # this was [batch_size, s2 - 2, s2 - 2, g_dim*1]
        W_conv3 = tf.get_variable('g_wconv3', [FILTER_SIZE, FILTER_SIZE, output3_shape[-1], int(H_conv2.get_shape()[-1])],
                                  initializer=tf.truncated_normal_initializer(stddev=0.1))
        b_conv3 = tf.get_variable(
            'g_bconv3', [output3_shape[-1]], initializer=tf.constant_initializer(.1))
        H_conv3 = tf.nn.conv2d_transpose(H_conv2, W_conv3, output_shape=output3_shape,
                                         strides=STRIDES, padding='SAME') + b_conv3
        ...
        W_conv4 = tf.get_variable('g_wconv4', [FILTER_SIZE, FILTER_SIZE, output4_shape[-1], int(H_conv3.get_shape()[-1])],
                                  initializer=tf.truncated_normal_initializer(stddev=0.1))
        b_conv4 = tf.get_variable(
            'g_bconv4', [output4_shape[-1]], initializer=tf.constant_initializer(.1))
        H_conv4 = tf.nn.conv2d_transpose(H_conv3, W_conv4, output_shape=output4_shape,
                                         strides=STRIDES, padding='VALID') + b_conv4
